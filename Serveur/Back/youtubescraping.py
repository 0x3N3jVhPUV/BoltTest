...
parser.add_argument('--channels_file', type=str, help='Path to channels.json file', default='./Crypto/youtube/subscriptions/channels.json')
parser.add_argument('--slack_channel', type=str, help='Slack channel name', default='crypto')
...
# File paths
CHANNELS_DIR = "./Crypto/youtube/subscriptions"
VIDEO_IDS_LOG = os.path.join(CHANNELS_DIR, "videosIds.txt")  # Updated file name
RESUMES_LOG = os.path.join(CHANNELS_DIR, "resumes.json")
LAST_CHANNEL_INDEX_FILE = os.path.join(CHANNELS_DIR, "last_channel_index.txt")
...
def load_channels():
    ...
    with open(CHANNELS_FILE, 'r') as file:
        data = json.load(file)
    channels = data.get(SLACK_CHANNEL, [])
    channel_ids = channels  # Assuming channels are already channel IDs
    print(f"Loaded {len(channel_ids)} channel IDs from category '{SLACK_CHANNEL}'.")
    return channel_ids
...
def format_summary(summary):
    ...
    # Supposons que les titres et sous-titres soient entourés de ** dans le texte brut
    formatted_summary = summary.replace("**", "<strong>").replace("**", "</strong>")
    # Ajouter des retours à la ligne
    formatted_summary = formatted_summary.replace("\n", "<br>")
    return formatted_summary

def save_resume(video_details, summary):
    """Save a generated summary to the log."""
    video_details['summary'] = format_summary(summary)  # Format the summary
    with open(RESUMES_LOG, 'a', encoding='utf-8') as file:
        json.dump(video_details, file, ensure_ascii=False)
        file.write("\n\n---\n\n")
...
def generate_summary(transcript):
    ...
    prompt = (
        ...
        f"{transcript}\n\n"
        f"Fait son résumé génral et concis pour un profane. Donne les conclusions à retenir et des exemples concrets. Efforce-toi d'être le plus détaillé possible pour les contenus courts "
...
def post_to_frontend(video_details):
    ...
    payload = {
        ...
        "has_subtitles": video_details['has_subtitles'],
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"Video details posted successfully for video {video_details['video_id']}.")
        else:
            print(f"Failed to post video details for video {video_details['video_id']}. Status code: {response.status_code}")
    except Exception as e:
        print(f"Error posting to frontend for video {video_details['video_id']}: {e}")

def process_and_post_videos(videos):
    """Process each video and post summaries to Slack and the frontend."""
    ...
    for video in videos:
        ...
        video['summary'] = summary  # Add summary to video details
        post_to_slack(video, summary)
        save_processed_id(video['video_id'])
        save_resume(video, summary)  # Save video details including summary and duration
        post_to_frontend(video)  # Post video details to the frontend
...
def main():
    ...
    while True:
        ...
        # subprocess.run(["python", "./Crypto/slack/remove_posts.py", SLACK_CHANNEL])
